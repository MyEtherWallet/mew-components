...
_Default_ is used to filter dates, and it has a blue background (named `blue500` in the MEW color palette) during the active state.
_Percentage_ is used to filter out percentages of a value and shows a green border (named `primary` in the MEW color palette) during the active state.
_Custom_ is selected when a user wants to use a custom button style, and it is created by using slots (**btn** + **number of btn**), i.e. _btn1_. The **buttonGroup** prop

 ...
 */
const loadingColor = () => {
  if (!props.disabled && isLight && isPrimaryTheme) {
    return 'greenPrimary';
  }

  if (!props.disabled && isLight && isSecondaryTheme) {
    return 'bluePrimary';
  }

  if (!props.disabled && isLight && isErrorTheme) {
    return 'redPrimary';
  }

  if (!props.disabled && isLight && isBasicTheme) {
    return 'greyPrimary';
  }

  if (!props.disabled && !isBackground) {
    return buttonColor;
  }
...
const buttonClasses = computed(() => {
  ...
  if (!disabled) {
    ...
    if (isErrorTheme && isLight) {
      classes.push('redPrimary--text');
    }

    if (isSecondaryTheme && isLight) {

...
watch(props.idxToExpand, (newVal, oldVal) => {
  if (newVal !== oldVal) {
...
const setToggle = (val) => {
  if (val === true && !expandIdxArr.value.includes(0)) {
    expandIdxArr.value.push(0);
  }
  if (val === false && expandIdxArr.value.includes(0)) {
    expandIdxArr.value.pop();
  }
  this.$refs.switch[0].setToggle(val);
};
const isExpanded = (idx) => {
  if (
    (Array.isArray(expandIdxArr.value) && expandIdxArr.value.includes(idx)) ||
    expandIdxArr.value === idx
  ) {
    return true;
  }
  return false;
